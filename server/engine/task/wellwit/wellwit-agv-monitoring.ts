...
async function WellwitAgvMonitoring(step, { logger }) {
  ...
  var monitoringHandler = messageData => {
  ...
  var interruptHandler = () => {
    connection.removeListener('agv-message', monitoringHandler)
  }

  connection.addListener('agv-message', monitoringHandler)
  connection.once('agv-interrupt', interruptHandler)

  while (true) {
    if (status === STATUS_FINISH || !socket) {
      connection.removeListener('agv-message', monitoringHandler)
      connection.removeListener('interupt', interruptHandler)
      break
    }
...
TaskRegistry.registerTaskHandler('wellwit-agv-monitoring', WellwitAgvMonitoring)
